...
export const Sidebar = ({ isMobile, onClose }: { isMobile?: boolean, onClose?: () => void }) => {
  ...
  return (
    <>
      <motion.aside
        initial={false}
        animate={{ width: isMobile ? "100%" : (isCollapsed ? 0 : 280) }}
        className={`bg-secondary/20 border-r border-border h-full flex flex-col relative transition-all duration-300 overflow-hidden ${isMobile ? "w-full" : "hidden md:flex"}`}
      >
        <div className={`p-6 flex flex-col h-full ${isMobile ? "w-full" : "w-[280px]"}`}>
          <div className="flex-1 overflow-y-auto space-y-2 scrollbar-hide">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold px-4 mb-4">Features</p>
